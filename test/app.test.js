...
const {seedAccounts} = require("./seed/accounts");
...
describe("POST /users", () => {
    ...
    it("should return a 200 and the user", (done) => {
        ...
        request(app)
        .post("/users")
        .send(seedAccounts.postUsers[0])
        .expect(res => {
            var {email, password, salt} = res.body;
            var sdEmail = seedAccounts.postUsers[0].email;
            var sdPassword = seedAccounts.postUsers[0].password;
...
describe("POST users/login", () => {
    ...
    before(() => {
        User.deleteMany({});
        User.create(seedAccounts.postUsersLogin[0]);
    });
    ...
    it("should return 200 and the user for logging in successfully", (done) => {
        request(app)
        .post("/users/login")
        .send(seedAccounts.postUsersLogin[0])
        .expect(res => {
            let {email, password} = res.body;
            let sdEmail = seedAccounts.postUsersLogin[0].email;
            let sdPassword = seedAccounts.postUsersLogin[0].password;
    ...
    it("should return 400 if user not found", (done) => {       
        request(app)
        .post("/users/login")
        .send(seedAccounts.postUsersLogin[1])
        .expect(res => {
